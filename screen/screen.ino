...
void handleSleep() {
  ...
  if (millis() - lastActionTime > 30000) {  // Check for inactivity (5 miniutes)
    goToSleep();
  }
}void setup() {
  ...
  Serial.println("Waiting for animation sequence to complete...");
  
  // Set initial last action time
  lastActionTime = millis();
  
  // IMPORTANT: Removed the forced mode change to INFO
  // Let the animation complete naturally, and DisplayController
  // will switch to HOME mode when isAnimationComplete() returns true
}
