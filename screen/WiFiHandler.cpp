...
// Method to set static IP configuration
void WiFiHandler::setStaticIP(IPAddress ip, IPAddress gateway, IPAddress subnet,
  ...
  Serial.println("Static IP configuration set: " + ip.toString());
}

// Get the current active IP (AP or STA)
IPAddress WiFiHandler::getCurrentIP() const {
  if (wifiConnected) {
    return WiFi.localIP();
    ...
    if (lastIndex >= 0 && lastIndex < 3) {
      ...
      if (!lastSSID.isEmpty()) {
        ...
        if (useStaticIP) {
          ...
          if (!WiFi.config(staticIP, gateway, subnetMask, dns1, dns2)) {
...
// Implementation of the single method for WiFi connection with static IP
bool WiFiHandler::connectToWifiWithStaticIP(const String& ssid, const String& password,
                                          IPAddress staticIP, IPAddress gateway,
                                          IPAddress subnet, IPAddress dns1, IPAddress dns2) {
  Serial.println("Connecting to WiFi with static IP configuration");
  ...
  // Switch to station mode while keeping AP if it's running
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\n✅ Successfully connected to WiFi with static IP!");
    Serial.print("IP address: ");
    Serial.println(WiFi.localIP());
    ...
    // Restart AP on the same channel as the connected WiFi if active
    if (apModeActive) {
      restartAPWithChannel();
    }
    ...
    // Set up OTA if we have a device name
...
bool WiFiHandler::startAPWithStaticIP(const char* ssid, const char* password, 
                                   IPAddress staticIP, IPAddress gateway, IPAddress subnet) {
  // Set AP mode
  WiFi.mode(WIFI_AP);
  
  // Configure the static IP for the access point
  ...
  Serial.println("AP mode started with static IP: " + staticIP.toString());
...
// A helper method to convert string IP addresses to IPAddress objects
IPAddress WiFiHandler::stringToIP(const String& ipStr) {
  IPAddress ip;
  if (!ip.fromString(ipStr)) {
    Serial.println("Invalid IP address format: " + ipStr);
    return IPAddress(0, 0, 0, 0);
  }
  return ip;
}
...
// Get WiFi signal strength as a 0-3 value
