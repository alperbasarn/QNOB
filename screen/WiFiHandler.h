...
public:
    ...
    IPAddress getCurrentIP() const;      // Get current IP address (AP or STA)
    bool isUsingStaticIP() const { return useStaticIP; }
    
    // New static IP methods
    bool connectToWifiWithStaticIP(const String& ssid, const String& password, 
                                 IPAddress staticIP, IPAddress gateway, 
                                 IPAddress subnet, IPAddress dns1 = IPAddress(8, 8, 8, 8), 
                                 IPAddress dns2 = IPAddress(8, 8, 4, 4));
    bool startAPWithStaticIP(const char* ssid, const char* password, 
